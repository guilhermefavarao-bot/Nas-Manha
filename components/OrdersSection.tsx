...
import { Printer, MessageSquare, Check, Search, Trash2, Wallet, CheckCircle2, CreditCard, Landmark, Banknote, X, AlertTriangle, FileSpreadsheet, Calendar, Loader2 } from 'lucide-react';
...
const OrdersSection: React.FC<Props> = ({ orders, onReady, onCloseOrder, onDelete }) => {
  ...
  const filteredOrders = useMemo(() => {
    return orders.filter(o => {
      ...
      return matchesSearch && matchesStatus && (statusFilter === 'fechado' ? matchesDate : true);
    });
  }, [orders, searchTerm, statusFilter, startDate, endDate]);

  const getQty = (item: any) => Number(item.qtd ?? item.qty) || 0;
  ...
  const handleExportXLSX = () => {
    ...
    if (closedOrders.length === 0) return alert("Nenhuma venda fechada neste período para exportar.");

    setIsExporting(true);
    try {
      const data = closedOrders.map(order => ({
        "ID": order.id,
        "Data": new Date(order.data).toLocaleDateString('pt-BR'),
        "Hora": new Date(order.data).toLocaleTimeString('pt-BR'),
        "Cliente": order.cliente,
        "Status": order.status.toUpperCase(),
        "Pagamento": order.pagamento || "N/A",
        "Total (R$)": Number(order.total).toFixed(2),
        "Itens": order.itens.map(i => `${getQty(i)}x ${i.nome}`).join(", ")
      }));

      const wb = XLSX.utils.book_new();
      const ws = XLSX.utils.json_to_sheet(data);
      XLSX.utils.book_append_sheet(wb, ws, "Vendas_Fechadas");
      XLSX.writeFile(wb, `VENDAS_ADEGA_${startDate}_A_${endDate}.xlsx`);
    } catch (e) {
  ...
  const handlePrint = (order: Order) => {
    ...
    const itensHtml = order.itens.map(item => {
      ...
      return `<div style="display: flex; justify-content: space-between; font-size: 13px; margin-bottom: 5px; font-family: monospace;"><span style="flex: 1;">${quantity}x ${item.nome}</span><span style="width: 80px; text-align: right;">R$ ${(quantity * price).toFixed(2)}</span></div>`;
    }).join('');

    const htmlContent = `<html><body style="font-family: monospace; width: 80mm; margin: 0 auto; padding: 10px;"><div style="text-align: center; border-bottom: 1px dashed #000; margin-bottom: 10px;"><h2>ADEGA NAS MANHA</h2><p>PEDIDO #${order.id.toString().slice(-4)}</p></div><div style="font-size: 12px; margin-bottom: 10px;"><strong>CLIENTE:</strong> ${order.cliente.toUpperCase()}<br><strong>DATA:</strong> ${new Date(order.data).toLocaleString()}</div>${itensHtml}<div style="font-size: 18px; font-weight: bold; text-align: right; margin-top: 10px; border-top: 1px dashed #000; padding-top: 5px;">TOTAL: R$ ${(Number(order.total) || 0).toFixed(2)}</div><script>window.onload=function(){window.print();window.close();}</script></body></html>`;
  ...
  const handleWhatsAppNotify = (order: Order) => {
    ...
    const itensFormatados = order.itens.map(item => `• ${getQty(item)}x ${item.nome}`).join('\n');
  ...
  const handleConfirmClosing = (orderId: number, type: string) => {
    ...
    if (!order) return;
    onCloseOrder(orderId, [{ type, value: Number(order.total) || 0 }]);
    setClosingOrderId(null);
  };

  return (
    <div className="space-y-6">
      <div className="bg-[#141414] p-5 rounded-[2rem] border border-zinc-800 shadow-xl space-y-4">
        ...
        {statusFilter === 'fechado' && (
          <div className="grid grid-cols-2 gap-3 animate-in fade-in slide-in-from-top-2">
            <div className="space-y-1">
              <label className="text-[8px] text-zinc-600 font-black uppercase ml-1">Vendas de:</label>
              <input type="date" value={startDate} onChange={e => setStartDate(e.target.value)} className="w-full bg-black border border-zinc-800 rounded-xl px-4 py-3 text-xs text-white outline-none focus:border-[#FFD700]" />
            </div>
            <div className="space-y-1">
              <label className="text-[8px] text-zinc-600 font-black uppercase ml-1">Até:</label>
              <input type="date" value={endDate} onChange={e => setEndDate(e.target.value)} className="w-full bg-black border border-zinc-800 rounded-xl px-4 py-3 text-xs text-white outline-none focus:border-[#FFD700]" />
            </div>
          </div>
        )}
        ...
        <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">
          {['aberto', 'pronto', 'fechado', 'todos'].map(status => (
            <button key={status} onClick={() => setStatusFilter(status as any)} className={`flex-1 min-w-[80px] py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest border transition-all ${statusFilter === status ? 'bg-[#FFD700] text-black border-[#FFD700] shadow-lg shadow-yellow-500/10' : 'bg-black text-zinc-600 border-zinc-900'}`}>{status}</button>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div key={order.id} className={`bg-[#141414] border-l-4 rounded-3xl p-6 shadow-2xl transition-all relative overflow-hidden ${order.status === 'fechado' ? 'border-green-500' : order.status === 'pronto' ? 'border-blue-500' : 'border-yellow-500'}`}>
            ...
            <div className="flex justify-between items-start mb-4">
              <div>
                ...
                <div className="flex items-center gap-2">
                  <span className="text-zinc-600 text-[9px] font-bold uppercase tracking-wider">#{order.id.toString().slice(-4)}</span>
                  <span className="text-zinc-700 text-[8px] font-bold">• {new Date(order.data).toLocaleString('pt-BR')}</span>
                </div>
              </div>
            ...
            <div className="space-y-2 mb-6 bg-black/40 p-4 rounded-2xl border border-zinc-900 min-h-[80px]">
              ...
              <div className="pt-3 mt-3 border-t border-zinc-800 flex justify-between items-center">
                ...
                <span className="text-[#FFD700] text-2xl font-black">R$ {(Number(order.total) || 0).toFixed(2)}</span>
              </div>
            ...
            <div className="flex flex-wrap gap-2">
              <button onClick={() => handlePrint(order)} className="p-4 bg-zinc-900 border border-zinc-800 rounded-2xl text-white flex justify-center items-center hover:bg-zinc-800 transition-colors" title="Imprimir Comanda">
              ...
              {confirmDeleteId === order.id ? (
                <button 
                  ...
                  className="px-6 bg-red-600 text-white rounded-2xl flex items-center gap-2 animate-pulse transition-all shadow-lg shadow-red-900/40"
                  onMouseLeave={() => setConfirmDeleteId(null)}
                >
                  <AlertTriangle className="w-4 h-4" />
                  <span className="text-[9px] font-black uppercase tracking-widest">Apagar Agora?</span>
                </button>
              ) : (
                <button 
                  onClick={() => setConfirmDeleteId(order.id)} 
                  className="p-4 bg-red-950/20 border border-red-900/20 rounded-2xl text-red-500 hover:bg-red-600 hover:text-white transition-all" 
              ...
              {order.status === 'pronto' && (
                <>
                  <button onClick={() => handleWhatsAppNotify(order)} className="flex-1 min-w-[80px] bg-green-600 hover:bg-green-500 text-white p-4 rounded-2xl flex justify-center gap-2 items-center transition-all shadow-lg shadow-green-900/20">
                    <MessageSquare className="w-5 h-5" />
                    <span className="hidden sm:inline text-[9px] font-black uppercase tracking-widest">Avisar</span>
                  </button>
                  
                  <button 
                    onClick={() => setClosingOrderId(order.id)} 
                    className="flex-[2] min-w-[120px] bg-[#ff5733] hover:bg-[#e64a2e] text-white p-4 rounded-2xl flex justify-center gap-2 items-center transition-all shadow-lg shadow-orange-900/20"
                  >
              ...
              {order.status === 'aberto' && (
                <button onClick={() => onReady(order.id)} className="flex-1 bg-blue-600 hover:bg-blue-500 p-4 rounded-2xl text-white flex justify-center items-center gap-3 transition-all">
              ...
              {order.status === 'fechado' && (
                <div className="flex-1 flex items-center justify-center p-4 bg-green-500/5 border border-green-500/10 rounded-2xl text-green-500/50 text-[10px] font-black uppercase tracking-widest">
                  Venda Concluída
                </div>
              )}
            </div>
        ...
        {filteredOrders.length === 0 && (
          <div className="col-span-full py-20 text-center border border-dashed border-zinc-900 rounded-[3rem]">
            <p className="text-zinc-700 font-bold uppercase tracking-[0.2em] text-[10px]">Nenhum pedido encontrado nesta categoria</p>
          </div>
        )}
      </div>
    </div>
